...
def addPublicationsToCv(templateSection, templateNewPaper, pubslistPath, citations=-1):
    # Open template files
    templateSectionRaw = open(templateSection, 'r').read()
    templateNewPaperRaw = open(templateNewPaper, 'r').read()
    ...
    if citations > 0:
        section.append((templateSectionRaw % (r'\textbf{Number of citations: %d}' % citations)).replace('currSection', ''))
...
def getCitations(dataPath):
    with open(dataPath, 'r') as fid:
        lines = fid.readlines()
        for l in lines:
            m = re.search("<numcit>(\d*)<\/numcit>", l.strip())
            if m and len(m.groups()) > 0:
                numcit = int(m.group(1))
                return numcit
    return -1
...
if __name__ == '__main__':
    ...
    # filenames
    pubsList = '/Users/giulio/Library/Mobile Documents/com~apple~CloudDocs/Education/7_Publications/publications.csv'
    ...
    dataPath = os.path.join(websitePath, 'data.html')
    ...
    # citations
    citations = getCitations(dataPath)
    ...
    # curriculum
    publications = addPublicationsToCv(templatePubsCvSection, templateNewPaperCv, pubsList, citations)
